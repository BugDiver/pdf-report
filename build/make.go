...
var deployDir = filepath.Join(deploy, pdfReport)
...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", pdfReport, getPluginVersion(), getGOOS(), getArch())
...
func compileGoPackage(packageName string) {
	runProcess("go", "build", "-o", getGaugeExecutablePath(pdfReport))
}
...
func copyPluginFiles(destDir string) {
	...
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), pdfReport+".exe")] = bin
	} else {
		files[filepath.Join(getBinDir(), pdfReport)] = bin
	}
	files[pluginJsonFile] = ""
	files[assetsDir] = assetsDir
...
func installPlugin(installPrefix string) {
	...
	pluginInstallPath := filepath.Join(installPrefix, pdfReport, getPluginVersion())
